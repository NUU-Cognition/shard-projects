...
const high = tasks.where(p => p.priority === 'high' && p.status !== 'done' && p.status !== 'review' && p.status !== 'deprecated')
...
const medium = tasks.where(p => p.priority === 'medium' && p.status !== 'done' && p.status !== 'review' && p.status !== 'deprecated')
  .array().sort((a, b) => statusOrder(a.status) - statusOrder(b.status));
if (medium.length === 0) {
  dv.paragraph("*None*");
} else {
...
const low = tasks.where(p => p.priority === 'low' && p.status !== 'done' && p.status !== 'review' && p.status !== 'deprecated')
  .array().sort((a, b) => statusOrder(a.status) - statusOrder(b.status));
if (low.length === 0) {
  dv.paragraph("*None*");
} else {
...
const noPriority = tasks.where(p => !p.priority && p.status !== 'done' && p.status !== 'review' && p.status !== 'deprecated')
  .array().sort((a, b) => statusOrder(a.status) - statusOrder(b.status));
